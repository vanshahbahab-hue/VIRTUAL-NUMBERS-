...
with app.app_context():
    ...
    # Add 100 virtual numbers (25 per country)
    numbers_data = {
...
def register():
    ...
    if existing:
        return render_template_string(LOGIN_HTML, error="Username exists!")
    ...
    return render_template_string(LOGIN_HTML, success="Account created! Please login.")
...
def dashboard():
    ...
    return render_template_string(DASHBOARD_HTML, user=user, numbers=numbers, orders=orders)
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Orbitron', monospace; }
        ...
        .login-card { position: relative; z-index: 1; background: rgba(10, 10, 42, 0.95); backdrop-filter: blur(15px); border-radius: 30px; padding: 45px 40px; width: 100%; max-width: 480px; border: 1px solid rgba(128,0,255,0.3); box-shadow: 0 0 50px rgba(128,0,255,0.15); }
        .logo { text-align: center; margin-bottom: 25px; }
        .logo img { max-width: 200px; }
        .sub { color: rgba(255,255,255,0.5); text-align: center; font-size: 12px; margin-bottom: 30px; letter-spacing: 3px; }
        ...
        .input-group input { width: 100%; padding: 15px 20px; background: rgba(255,255,255,0.05); border: 1px solid rgba(128,0,255,0.3); border-radius: 15px; color: white; font-size: 14px; letter-spacing: 1px; }
        .input-group input:focus { outline: none; border-color: #aa00ff; box-shadow: 0 0 20px rgba(170,0,255,0.3); }
        .btn { width: 100%; padding: 15px; background: linear-gradient(135deg, #aa00ff, #6600cc); border: none; border-radius: 15px; color: white; font-size: 16px; font-weight: 800; letter-spacing: 2px; cursor: pointer; margin-top: 10px; transition: 0.3s; }
        .btn:hover { transform: translateY(-3px); box-shadow: 0 8px 25px rgba(170,0,255,0.4); }
        ...
        .divider { text-align: center; margin: 25px 0; color: rgba(255,255,255,0.3); font-size: 11px; position: relative; }
        ...
        a { color: #aa00ff; text-decoration: none; }
    </style>
</head>
<body>
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Dashboard | Virtual Numbers</title>
    <style>
        @import url('https://fonts.googleapis.com/css2?family=Orbitron:wght@400;500;600;700;800;900&display=swap');
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Orbitron', monospace; }
        body { background: linear-gradient(135deg, #0a0a2a, #1a0a3a, #2a1a4a); min-height: 100vh; }
        .navbar { background: rgba(10, 10, 42, 0.95); backdrop-filter: blur(10px); padding: 15px 30px; display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid #aa00ff; position: sticky; top: 0; z-index: 100; }
        .logo img { height: 50px; }
        .wallet { background: linear-gradient(135deg, #aa00ff, #6600cc); padding: 10px 25px; border-radius: 30px; color: white; font-weight: 800; letter-spacing: 1px; }
        .menu-btn { background: none; border: none; cursor: pointer; display: flex; flex-direction: column; gap: 5px; }
        .menu-btn span { width: 25px; height: 2px; background: #aa00ff; transition: 0.3s; }
        ...
        .sidebar a { display: block; color: white; text-decoration: none; padding: 15px; margin: 10px 0; border-radius: 10px; transition: 0.3s; font-size: 14px; letter-spacing: 1px; }
        ...
        .welcome { color: #aa00ff; margin-bottom: 30px; font-size: 24px; font-weight: 700; letter-spacing: 2px; }
        ...
        .stat-card h3 { color: rgba(255,255,255,0.6); font-size: 12px; letter-spacing: 2px; }
        .stat-card .value { color: #aa00ff; font-size: 32px; font-weight: 800; }
        .section-title { color: #aa00ff; margin: 30px 0 20px; font-size: 20px; letter-spacing: 2px; }
        ...
        .country-header h3 { color: #aa00ff; font-size: 18px; letter-spacing: 2px; }
        .numbers-list { max-height: 500px; overflow-y: auto; }
        .number-item { display: flex; justify-content: space-between; align-items: center; padding: 12px; margin: 8px 0; background: rgba(0,0,0,0.3); border-radius: 10px; transition: 0.3s; }
        .number-item:hover { background: rgba(170,0,255,0.1); }
        ...
        .number-info span { color: white; font-size: 13px; letter-spacing: 1px; }
        ...
        .buy-btn { background: linear-gradient(135deg, #aa00ff, #6600cc); border: none; padding: 6px 15px; border-radius: 20px; color: white; font-weight: bold; cursor: pointer; font-size: 12px; transition: 0.3s; }
        ...
        .modal-content h2 { color: #aa00ff; margin-bottom: 20px; font-size: 24px; letter-spacing: 2px; }
        .modal-content input { width: 100%; padding: 14px; margin: 12px 0; background: rgba(255,255,255,0.08); border: 1px solid rgba(170,0,255,0.3); border-radius: 12px; color: white; font-size: 14px; }
        .modal-content button { width: 100%; padding: 14px; background: linear-gradient(135deg, #aa00ff, #6600cc); border: none; border-radius: 12px; color: white; font-weight: 800; cursor: pointer; margin: 5px 0; }
...
<body>
    <div class="navbar">
        ...
        <div style="display: flex; align-items: center; gap: 20px;">
            <div class="wallet">💰 ₹{{ "%.2f"|format(user.balance) }}</div>
    ...
    <div class="sidebar" id="sidebar">
        ...
        <a href="#" onclick="openAddCash()">💰 ADD FUNDS</a>
        <a href="/order-history">📜 MY ORDERS</a>
        <a href="/transaction-history">📊 TRANSACTIONS</a>
    ...
    <div class="container">
        ...
        <div class="countries">
            {% set countries = {'USA': {'price': 320, 'flag': 'https://i.ibb.co/pv1YDt7X/photo-AQADIh-Br-G81-GYFV.jpg', 'prefix': '+1'}, 'CANADA': {'price': 365, 'flag': 'https://i.ibb.co/DHJcy546/photo-AQADIx-Br-G81-GYFVy.jpg', 'prefix': '+1'}, 'UAE': {'price': 385, 'flag': 'https://i.ibb.co/cS23Q3S3/photo-AQADJBBr-G81-GYFVy.jpg', 'prefix': '+971'}, 'AUSTRALIA': {'price': 325, 'flag': 'https://i.ibb.co/CKNrdykP/photo-AQADIRBr-G81-GYFVy.jpg', 'prefix': '+61'} } %}
            ...
            <div class="country-section">
                ...
                <div class="numbers-list">
                    {% for num in numbers if num.country == country and not num.is_sold %}
                    <div class="number-item">
                        <div class="number-info">
                            <img src="{{ data.flag }}" alt="">
                            <span>{{ num.number }}</span>
                            <div class="price-detail">telegram, whatsapp, any app login</div>
                        </div>
                        <div class="number-price">₹{{ num.price }}</div>
                        <button class="buy-btn" onclick="buyNumber({{ num.id }}, {{ num.price }})">BUY</button>
                    </div>
                    {% endfor %}
                </div>
            </div>
        ...
        <div class="table-container">
            <table>                <thead><tr><th>ORDER ID</th><th>NUMBER</th><th>COUNTRY</th><th>AMOUNT</th><th>STATUS</th><th>DATE</th></tr></thead>
                <tbody>
                    {% for o in orders %}
                    <tr>
                        <td style="font-family:monospace;">{{ o.order_id }}</td>
                        <td>{{ o.number }}</td>
                        <td>{{ o.country }}</td>
                        <td>₹{{ o.amount }}</td>
                        <td style="color:#00ff00;">{{ o.status }}</td>
    ...
    <div id="addCashModal" class="modal">
        <div class="modal-content">
            <h2>ADD FUNDS</h2>
            <p style="color:#ffaa00; margin-bottom:10px;">Minimum Deposit: ₹349</p>
            <input type="number" id="cashAmount" placeholder="AMOUNT (₹349 - ₹10000)" min="349" max="10000">
    ...
    <div id="notifModal" class="modal">
        <div class="modal-content">
            <h2>NOTIFICATIONS</h2>
            <div id="notifContent" style="color:white;">No new notifications</div>
            <button class="close-btn" onclick="closeNotif()">CLOSE</button>
        </div>
    </div>
    ...
    <script>
        ...
        function buyNumber(id, price) {
            ...
            if(balance < price) {
                showToast('⚠️ INSUFFICIENT FUNDS! Need ₹' + price + '. Please add funds to wallet.', false);
                setTimeout(() => { openAddCash(); }, 1500);
                return;
            }
            
            if(confirm(`Buy this virtual number for ₹${price}?`)) {
        ...
        function generateQR() {
            ...
            if(amount < 349) { showToast('Minimum deposit is ₹349', false); return; }
            if(amount > 10000) { showToast('Maximum deposit is ₹10000', false); return; }
            fetch('/add-cash', { method: 'POST', headers: {'Content-Type': 'application/x-www-form-urlencoded'}, body: 'amount=' + amount })
            .then(res => res.json()).then(data => {
                if(data.success) {
                    document.getElementById('qrResult').innerHTML = `
                        <p style="color:#00ffcc;">PAY: ₹${data.qr_amount}</p>
                        <img src="data:image/png;base64,${data.qr_code}" style="width:200px;">
                        <p style="color:#ffaa00;">TXN ID: ${data.transaction_id}</p>
                        <p style="color:#aaa;">UPI: v76009423@oksbi</p>
                        <p style="color:#aaa; font-size:11px;">Pay exact amount and enter transaction ID below</p>
                        <input type="text" id="verifyTxn" placeholder="Enter Transaction ID" style="width:100%; padding:10px; margin:10px 0;">
                        <button onclick="verifyPayment('${data.transaction_id}')">VERIFY PAYMENT</button>
                    `;
        ...
        function verifyPayment(txnId) {
            let userTxn = document.getElementById('verifyTxn').value;
            if(!userTxn) { showToast('Enter Transaction ID', false); return; }
            fetch('/verify-payment', { method: 'POST', headers: {'Content-Type': 'application/x-www-form-urlencoded'}, body: 'transaction_id=' + userTxn })
            .then(res => res.json()).then(data => {
                if(data.success) {
                    showToast('✅ Payment verified! ₹' + data.balance + ' added to wallet', true);
                    setTimeout(() => { location.reload(); }, 1500);
                } else { showToast('Error: ' + data.error, false); }
            });
        }
        ...
        function showNotifications() {
            ...
            document.getElementById('notifContent').innerHTML = '<p>🔔 Welcome to Virtual Numbers!<br>Minimum deposit: ₹349</p><p>📞 Virtual numbers for Telegram, WhatsApp, and other app verifications.</p>';
        }
...
<style>@import url('https://fonts.googleapis.com/css2?family=Orbitron:wght@400;500;600;700;800;900&display=swap');*{margin:0;padding:0;box-sizing:border-box;font-family:'Orbitron',monospace;}body{background:linear-gradient(135deg,#0a0a2a,#1a0a3a,#2a1a4a);}.navbar{background:rgba(10,10,42,0.95);padding:15px 30px;display:flex;justify-content:space-between;border-bottom:1px solid #aa00ff;}.logo img{height:50px;}.container{padding:30px;}.stats{display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:20px;margin-bottom:30px;}.stat-card{background:rgba(255,255,255,0.05);border:1px solid #aa00ff;border-radius:20px;padding:25px;text-align:center;}.stat-card h3{color:rgba(255,255,255,0.6);font-size:12px;}.stat-card .value{color:#aa00ff;font-size:28px;font-weight:bold;}.table-container{background:rgba(255,255,255,0.05);border-radius:16px;overflow-x:auto;margin:20px 0;}table{width:100%;border-collapse:collapse;}th,td{padding:10px;color:white;border-bottom:1px solid rgba(255,255,255,0.1);font-size:12px;}th{color:#aa00ff;}button{background:#aa00ff;border:none;padding:5px 10px;border-radius:5px;cursor:pointer;font-weight:bold;color:white;}</style>
</head>
<body><div class="navbar"><div class="logo"><img src="https://i.ibb.co/4cHCQB4/photo-AQADKx-Br-G81-GYFV.jpg"></div><a href="/logout" style="color:#aa00ff;">LOGOUT</a></div>
